...
class WechatAuth {
    ...
    public function __construct($appid, $secret, $token = null){
        if($appid && $secret){
            ...
            throw new \think\Exception('缺少参数 APP_ID 和 APP_SECRET!');
        }
    }
    ...
    public function getAccessToken($type = 'client', $code = null){
        ...
        switch ($type) {
            ...
            default:
                throw new \think\Exception('不支持的grant_type类型！');
                break;
        }
        ...
        if(is_array($token)){
            if(isset($token['errcode'])){
                throw new \think\Exception($token['errmsg']);
            } else {
                $this->accessToken = $token['access_token'];
                return $token;
            }
        } else {
            throw new \think\Exception('获取微信access_token失败！');
        }
    }
    ...
    public function mediaUpload($filename, $type){
        ...
        if(!$filename) throw new \think\Exception('资源路径错误！');
    ...
    protected static function http($url, $param, $data = '', $method = 'GET'){
        ...
        //发生错误，抛出异常
        if($error) throw new \think\Exception('请求发生错误：' . $error);
